...
function toNormalizedTokenList(value) {
    if (Array.isArray(value)) {
        return value.map((entry) => normalizeToken(entry)).filter((entry) => entry && entry !== '');
    }
    const token = normalizeToken(value);
    return token ? [token] : [];
}
...
function evaluateItemVisibility(item = {}, filters = {}) {
    ...
    const normalizedEffectSearches = Array.isArray(effectSearches)
        ? effectSearches.map((entry) => normalizeEffectSearchEntry(entry)).filter((entry) => entry !== null)
        : [];
    ...
    const normalizedTagFallbackTerms = normalizedTagTerms.length ? normalizedTagTerms : normalizeTagSearchTerms(tagTerm);
...
export function registerFilterPredicatesModule(target = typeof globalThis !== 'undefined' ? globalThis : undefined) {
    if (!target) {
        return null;
    }
    ...
    namespace.filterPredicates = module;
    return module;
}

registerFilterPredicatesModule();
