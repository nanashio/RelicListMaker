(() => {
    const moduleResolvers =
        typeof window !== 'undefined' && window && window.galleryModules && window.galleryModules.tagTokenResolvers
            ? window.galleryModules.tagTokenResolvers
            : null;

    const fallbackDefaultParseTagTokens = function defaultParseTagTokens(value) {
    ...
    function createResolveTagTokenParsers(defaultParseTokenFn) {
        return function resolveTagTokenParsers(config = {}) {
            ...
            const defaultParserCandidate =
                typeof config.defaultParseTagTokens === 'function'
                    ? config.defaultParseTagTokens
                    : (namespace && namespace.defaultParseTagTokens) || defaultParseTokenFn;
    ...
    const defaultParseTagTokens = moduleResolvers?.defaultParseTagTokens || fallbackDefaultParseTagTokens;
    const resolveTagTokenParsers =
        moduleResolvers?.resolveTagTokenParsers || createResolveTagTokenParsers(defaultParseTagTokens);
