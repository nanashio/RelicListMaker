...
## テスト方針
pytest を 推奨 し `tests/` に モジュール 対応 テスト (`tests/test_match_and_export.py` など) を 用意 します。データ を フィクスチャ 共有 し、Tesseract 呼び出し は モック 化 して CI で 安定 動作 を 目指します。OCR マッチング、CSV スキーマ、HTML 生成 は 決定 論的 アサーション で 検証 し、チェック が 必要 な 項目 は PR に 手順 を 記録。
