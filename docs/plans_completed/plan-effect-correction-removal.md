...
- ✅ フェーズ 1-4: `_apply_corrections` の互換ガードを撤去し、`merge_results` が補助列を検出した場合はマイグレーション不足として例外を投げるようにしました。これにより旧 CSV のまま統合処理へ進むことを防ぎ、手動で基列へ転記するまで処理を停止できます。【F:relic_cli/commands/merge_results/rows.py†L38-L55】【F:tests/test_merge_results.py†L258-L305】
- ✅ フェーズ 2-1: 既存 CSV が存在しないことを確認済みで、追加マイグレーションは不要と判断しました。検証用に試作した移行スクリプトは保守対象から外し、補助列付き CSV が見つかった場合は手動での転記・削除を前提とします。【F:README.md†L74-L86】
...
- ✅ フェーズ 3-1, 3-2: 補助列 `Effect{n}Correction` / `Effect{n}LevelCorrection` / `Demerit{n}Correction` の参照をフロントエンド・バックエンド双方から削除し、ビューアが基列へ直接書き込む実装へ更新しました。`recordActionHandlers` は補助列を触らずに `Effect{n}` / `Effect{n}Level` / `Demerit{n}` を更新し、`merge_results` と GUI のレビュー判定も基列のみを参照するよう統一しています。【F:templates/gallery/render/effectViewModel.js†L1-L188】【F:templates/gallery/render/effectFactory.js†L90-L1028】【F:templates/gallery/events/recordActionHandlers.js†L240-L660】【F:relic_cli/commands/merge_results/rows.py†L58-L96】【F:gui/handlers.py†L19-L52】【F:tests/js/gallery_modules.test.mjs†L1758-L3404】【F:tests/test_merge_results.py†L232-L324】
- ✅ フェーズ 3-2 フォローアップ: 効果値が既存値から変化していない場合は `Effect{n}Status` のレビュー状態を維持するようにし、手動補正を解除したときのみ未レビューへ戻る挙動を保証しました。これにより補正列廃止後も `only_reviewed=True` の統合作業でレビュー済み行が欠落しないことを確認しています。【F:templates/gallery/events/recordActionHandlers.js†L350-L460】【F:relic_cli/commands/merge_results/rows.py†L58-L96】
- ✅ フェーズ 3-4: 補助列依存のユーティリティとテストシナリオを整理し、レビュー済み CSV が基列のみで成立することを pytest / Node.js テストで検証しました。旧形式 CSV が見つかった場合は手動で基列へ転記する運用に切り替え、新しいサンプルデータと統合テストを基列主体に刷新しています。【F:relic_cli/commands/merge_results/rows.py†L58-L96】【F:tests/test_merge_results.py†L40-L324】【F:tests/js/gallery_modules.test.mjs†L1758-L3404】【F:README.md†L74-L86】
...
- ✅ 次のアクション (2025-11-15): README にレビュー手順と補助列クリーンアップ方針を追加し、ビューアが基列を直接更新する運用と手動転記の手順を共有しました。【F:README.md†L29-L36】【F:README.md†L74-L86】
...
4. `relic_cli.commands.merge_results.merge` では補助列を最終 CSV へ書き戻さないようガードを追加し、`Effect{n}` 系列への直接マージ結果を検証する。

### フェーズ 2: データマイグレーションとテスト強化
1. 過去 CSV に残存する補助列値を `Effect{n}` 等へ反映するため、必要に応じてワンオフの移行処理を実施する。既にレビュー CSV を含む過去資産が存在しないケースでは、このステップは省略可能です。
...
2. フロントエンド (`templates/gallery/render/effectFactory.js` など) およびバックエンド (`relic_cli.commands.merge_results.merge`, `relic_data.py`) から補助列に関するフィールド、定数、バリデーションを削除する。
...
- 移行作業は冪等にし、実行前に対象ファイルのバックアップ (`*.bak`) を生成する。
...
- ✅ 補助列付き CSV が投入された際のエラー解消フローを README にまとめ、手動転記での解消手順を共有しました。【F:README.md†L74-L86】
