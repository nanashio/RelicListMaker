...
- リファクタリング後に優先したいテスト例:
  ...
  - **OCR 設定の回帰テスト**: サンプル画像を使って `pytest tests/ocr/`（必要に応じて新設）で `match_and_export.py` の推論結果を検証。
...
- **次のアクション** 例: `match_and_export.py` の新アルゴリズム AB テスト、Playwright シナリオ追加など具体的な一手を記載。
