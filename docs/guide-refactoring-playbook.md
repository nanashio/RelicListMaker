...
- リファクタリング開始前と完了後のコミット番号を計画書に明記し、コード量・可読性・変更容易性（必要に応じて追加指標も計画書に記載）で評価する。評価手順と結果の記録欄を計画書に設け、レビュー時に参照できるようにする。
...
## 7. ペンディング事項テンプレート
未着手や調査中の項目はコミット番号付きテンプレートで管理し、進捗に合わせて更新する。表の書式と記入例は `docs/guide-refactoring-progress-log.md` の「ペンディング事項テンプレート」セクションを参照し、同じフォーマットを計画書に貼り付けて使うこと。
