...
- ✅ フェーズ 2-4: 補助列未依存の Playwright シナリオを追加し、効果名を直接編集した際に保存 API が補助列を含まないペイロードを送信することと、基列 `Effect{n}` / `Effect{n}Status` が更新されることを検証しました。【F:tests/browser/viewer.spec.ts†L99-L159】
