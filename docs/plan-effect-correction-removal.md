...
- ⚠️ フェーズ 2-1: 既存 CSV を補正適用後の形式へ一括変換するマイグレーションスクリプトは未着手。要件定義と実装計画が必要です。
...
1. 過去 CSV に残存する補助列値を `Effect{n}` 等へ反映するスクリプト (例: `scripts/migrate_effect_corrections.py`) を追加し、既存資産をクリーニングする。
