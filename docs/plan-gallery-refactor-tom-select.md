...
## 実行計画（4 スプリント想定）
- **スプリント 1: TomSelect アダプタ** — タグ入力/検索の生成・破棄・設定共有をアダプタに集約し、デフォルトフォールバック込みで既存 UI と同等動作を維持。
- **スプリント 2: タグ同期とイベント束ね** — タグ正規化と保存をストア API に移し、`tagInputEvents` で編集状態と同期処理を単一点化。
...
- ### フォローアップ 1: ブラウザ E2E の再実行と証跡取得
  - 進捗: 0%（ネットワーク許可待ち。Playwright のブラウザ取得失敗により未実施）
...
- ### フォローアップ 3: 共有リゾルバのカバレッジ拡充
  - 進捗: 0%（未着手。E2E/Node 追加テストの具体化が未実施）
...
- ### フォローアップ 5: タグトークン共有リゾルバのモジュール化と依存明示
  - 進捗: 0%（未着手。ESM 版の作成とコピー監視の更新が未開始）
