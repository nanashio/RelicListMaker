...
## この文書の構成
- 背景とゴール: 現状の課題と対象範囲を確認するためのサマリー。
- スコープ外・前提: 対象外と依存前提の整理。
- 改善提案: TomSelect まわりとテンプレート全般の優先整理ポイント。
- 実行計画とフォローアップ: スプリントとフォローアップの統合表。
- リスクと緩和策 / テスト運用ルール: 進め方と検証方針の整理。
- 成果確認チェックリスト: 完了判定と根拠の一覧。
- 付録: A 進捗サマリー（現行ステータス）、B 日次ログ、C 詳細メモ。
...
| ID | 区分 | 目的/背景 | 主なタスク | 状況 | 成果物/メモ |
| --- | --- | --- | --- | --- | --- |
| S1 | スプリント | タグ入力/検索の生成・破棄・設定共有をアダプタに集約し、フォールバック込みで既存 UI と同等動作を維持 | TomSelect 生成・設定の共通化、デフォルトフォールバックの同梱、入力/検索コントローラーへの依存注入 | 完了（3/3） | TomSelect アダプタ導入と共有設定の一元化 |
| S2 | スプリント | タグ正規化と保存をストア API に移し、イベント束ねで編集状態を一元管理 | タグストア導入、`tagInputEvents` での input/focus/change 集約、UI 同期のストア依存化 | 完了（3/3） | タグ同期のストア移譲とイベント集約 |
...
| F1 | フォローアップ | Playwright ブラウザ取得不可な環境に配慮しつつ、E2E カバレッジを確保 | `npm run test:browser -- tests/browser/viewer.spec.ts` をローカルで実行し、TomSelect 差し替えとタグトークンフォールバックを UI 上で確認 | 100%（ローカル完了、Codex Web 環境は常時スキップ） | ローカル実行ログとスクリーンショットを記録、環境差分を明記 |
...
| F3 | フォローアップ | `sharedResolvers` のフォールバック経路を E2E でも確認しカバレッジを埋める | `tests/js/gallery_modules.test.mjs` でフォールバックケース追加、`npm run test:browser` で UI 上の経路を確認 | 90%（Node 追加検証済み、ブラウザ取得待ち） | 追加テストとフォールバック通過ログを記録 |
...
した。【F:templates/gallery/components/tomSelectAdapterFactory.js†L52-L82】【F:tests/js/gallery_modules.test.mjs†L188-L243】
  ...
  - タグトークンパーサーブリッジが `resolveDefaultParseTagTokensBridge` を利用してデフォルトパーサーを解決するように調整し、コンポーネントと共有リゾルバ間で同一のフォールバックチェーンを利用するよう統一。Node テストでブリッジ経由の解決を検証。
...
## 進捗メモ（2025-12-06）
- タグトークンのデフォルト解決を共通ブリッジに寄せて負債を削減。
  - `templates/gallery/components/tagTokenDefaults.js` を追加し、`resolveDefaultParseTagTokensBridge` でタグ入力/タグ検索が同一のデフォ
    ルトパーサーとフォールバックを参照するよう統一。`tagInput.js` と `tagSearch.js` は新ブリッジを優先し、共有リゾルバ未読込時も同じ
    デフォルト関数へフォールバックするよう整理。
  - 生成物コピーと依存リスト（`gallery/assets.py`、`templates/gallery/index.js`、`tests/test_generate_gallery.py`）に新モジュールを
    追加し、漏れを Node テストで検知できるようにした。`tests/js/gallery_modules.test.mjs` にデフォルトパーサーブリッジの単体テストを
    追加し、共有リゾルバ・明示指定・内蔵フォールバックのいずれも確認。
- テスト: `npm run test:node` を実行。
