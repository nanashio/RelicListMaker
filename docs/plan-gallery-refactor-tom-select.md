...
- スプリント別の完了タスク数
  ...
  - フォローアップ: デバッグフラグ解決を `tagDebug` モジュールに集約し、タグ入力/検索/ビューで共有済み。TomSelect 共有リゾルバのフォールバックを強化し、ロード順に依存せずデフォルトアダプタへ接続できるようにした。タグ検索のフォールバック経路でも `tagTokens` モジュールのパーサーを自動採用し、タグ入力と検索の正規化ロジックの差異を解消。
  - 新規フォローアップ: タグトークンパーサーを `sharedResolvers` の共有リゾルバで統一し、タグ検索・フォールバック経路とも同一の正規化ロジックを使うよう固定化。共有リゾルバ経由の解決を Node テストで検証し、`galleryModules.tagTokens` がない場合のフォールバックも合わせて記録。
  - 追加フォローアップ: タグ入力コンポーネントも共有リゾルバ経由でタグトークンパーサーを解決し、タグ入力/検索/ビューの正規化経路を一本化。フォールバック時も `tagTokens` モジュールを優先しつつ共通のデフォルトパーサーへ戻るようにした。
