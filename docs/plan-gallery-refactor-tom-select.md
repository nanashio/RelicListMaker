...
- スプリント完了状況と残り
  ...
  - ⏳ スプリント 4（テスト整備・ESM 化の足場）: タグトークン正規化を ES Module 化し、フィルター評価ロジックを `filterPredicates` モジュールとして抽出・共有。コピー対象と Node テストを更新済み。残りはフィルター純粋関数の追加ケース拡充。
- スプリント別の完了タスク数
