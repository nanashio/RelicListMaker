...
3. 🚧 CLI 実行は `RelicListMakerCLI.exe` に統一し、GUI は `RelicListMaker.exe` として配布する構成で移行中。PyInstaller の windowed/console 設定が排他的なため 1 本化は行わない方針。既存のタスクスケジューラや自動化設定を順次 CLI 版へ差し替え、`match_and_export.py` 削除の可否は移行完了後に判断する。【F:pyinstaller.spec†L75-L162】【F:docs/guide-pyinstaller-windows.md†L38-L76】
