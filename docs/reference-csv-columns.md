...
| `Effect{n}LevelCorrection` | レベル訂正入力欄 (非推奨)。 | ○（列フラグで制御） | 列表示フラグ `LevelCorrection` が有効なときに空欄で生成されるが、フェーズ 1 ではビューア入力欄を非表示化し、直接 `Effect{n}Level` を編集する。既存 CSV の値は今後の移行フェーズでクリーニングする予定。【F:relic_pipeline/io/exporter.py†L91-L192】【F:relic_pipeline/io/exporter.py†L287-L290】【F:templates/gallery/render/effectFactory.js†L789-L807】【F:templates/gallery/events/recordActionHandlers.js†L404-L520】【F:templates/gallery/gallery.js†L2138-L2194】 |
| `Effect{n}Correction` | 効果名の訂正入力欄 (非推奨)。 | △（ビューア編集時に追加） | フェーズ 1 では UI から補助入力を隠し、直接 `Effect{n}` を更新する実装へ移行した。既存の補助列は参照のみとし、新規値の書き戻しは行わない。【F:templates/gallery/render/effectFactory.js†L789-L807】【F:templates/gallery/events/recordActionHandlers.js†L404-L520】【F:templates/gallery/gallery.js†L2138-L2170】【F:merge_results.py†L257-L272】 |
...
| `Demerit{n}Correction` | デメリット名の訂正入力欄 (非推奨)。 | △（ビューア編集時に追加） | フェーズ 1 では補助入力を無効化し、デメリット名は `Demerit{n}` に直接保存する。補助列は既存データ互換のために残存するのみ。【F:templates/gallery/render/effectFactory.js†L288-L356】【F:templates/gallery/render/effectFactory.js†L789-L807】【F:templates/gallery/events/recordActionHandlers.js†L404-L520】【F:templates/gallery/gallery.js†L2138-L2170】 |
