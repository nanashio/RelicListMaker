...
| `Tags` | レビュー担当者が任意に付与できるタグ一覧。 | ○（列フラグで制御） | `build_row` が空文字で初期化し、列フラグが有効な場合に `write_csv` が `Tags` 列をヘッダーへ追加する。保存時には `write_csv`・ビューアの `generateCsv`・`viewer_server.storage.write_records` がタグ値を正規化し、スペースやカンマで入力された値もセミコロン区切り（`tag-a;tag-b`）に統一する。`galleryDataUtils.serializeTagTokens` が同じ区切りでの書き出し/読込を支える。【F:relic_pipeline/io/exporter.py†L18-L280】【F:templates/gallery/gallery.js†L1508-L2010】【F:viewer_server/storage.py†L1-L160】【F:templates/gallery/utils/data.js†L320-L390】 | アイテム左列のタグ入力欄とバッジ表示で編集・閲覧でき、入力したタグは検索キャッシュにも取り込まれる。【F:templates/gallery/render/itemFactory.js†L214-L330】【F:templates/gallery/render/galleryView.js†L520-L740】【F:templates/gallery/events/recordActionHandlers.js†L5-L420】 |
...
| `Demerit{n}Status` | デメリットのレビュー状況。適用対象外の場合は `none`。 | ○（ビューアで更新） | デメリット補正欄に入力が入ると `corrected` が適用され、空に戻すと `pending` へ戻る。【F:templates/gallery/render/effectViewModel.js†L24-L103】【F:templates/gallery/events/recordActionHandlers.js†L432-L620】 パスボタンは `pending`⇔`pass` を切り替え、承認すると関連する訂正値を消去した上で保存をスケジュールする。CSV生成時は通常遺物などデメリットが不要な行を `_initial_demerit_status` で `none` に初期化し、ビューア側もデメリットルール未適用時や通常遺物への切替時に `none` を自動付与して未レビュー件数へ含めない。【F:relic_pipeline/io/exporter.py†L32-L92】【F:templates/gallery/render/effectFactory.js†L309-L580】 | デメリットカードのステータス表示とボタン状態に反映。`none` の場合はボタンが選択されず、対象外としてカウントされない。【F:templates/gallery/render/effectFactory.js†L612-L855】【F:templates/gallery/gallery.js†L533-L585】【F:templates/gallery/render/galleryView.js†L200-L415】 |
