...
- 変更を加えたら必ず `npm run test:all` を実行し、Python・Node・Playwright の総合テストで回帰が無いことを確認する。`docs/gallery-refactor-plan.md` の進捗ログにもテスト結果の要点を記録する。
...
- Playwright を利用できない環境では、回帰確認の代替フローとして `pytest` と `node --test tests/js/gallery_modules.test.mjs` を続けて実行することを推奨する。代替フローを採用した場合は `docs/gallery-refactor-plan.md` の「フォーカスすべき次アクション」に注記を追加して整合性を保つ。
